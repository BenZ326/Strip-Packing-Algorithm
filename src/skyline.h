...
namespace StripPacking
{
	...
	public:
		Skyline(const int t_stripW) :
			corX(0), corY(0), length(t_stripW),
			next(nullptr), prev(nullptr){}
		Skyline() {}
		Skyline(const bool t_dummy) :
			corX(-1), corY(999999), length(0),
			next(nullptr), prev(nullptr) {}
		int corX;
		int corY;
		int length;
	...
	Skyline* selectSkyline(const Skyline* t_head, const skylineSelectionMode & t_mode);
	...
	void liftSkyline(Skyline* t_skyline);				// lift a skyline to the adjacent  skyline that has lower height
