...
const std::vector<const StripPacking::item*> StripPacking::BLEU::preprocess4yCheck(int& t_Width, const std::vector<const item*>& t_InterestItems, std::vector<coordinate>& t_Cords,
	const int t_Height) const
{
	auto Items = this->preprocessedFirst4yCheck(t_InterestItems, t_Cords, t_Width);
...
const std::vector<StripPacking::item*> StripPacking::BLEU::preprocessedFirst4yCheck(const std::vector<const item*>& t_InterestItems,
...
*/
void StripPacking::BLEU::preprocessedSecond4yCheck(std::vector<item*>& t_allItems,
...
*/
void StripPacking::BLEU::preprocessedThird4yCheck(std::vector<item*>& t_allItems,
	std::vector<coordinate>& t_Cords, int& t_binWidth) const
{
		...
		int& t_binWidth)
	{
		...
		// x coordinate of the items in ItemsAfterEndCol should should be reduced by t_eCol - t_sCol 
		// t_binWidth -= t_eCol - t_sCol
	...
	while (cur < t_binWidth)
	{
		...
		while (probe < t_binWidth)
		{
			...
			const auto&& occItemsProb = this->getItemsByCol(probe, t_allItems, t_Cords);
			// if current column can be merged with the column being probed, continue, go on probing
			if (occItemsCur == occItemsProb) { probe++; }
			else break;
		}
		...
		if (probe - cur > 1) 
		{
			...
			for (const auto& it : occItemsCur) ItemsInCol.push_back(idxHelper_Items.find(it)->second);
			std::vector<item*> ItemsAfterEndCol;
			for (const auto& it : t_allItems)
			{
				if (t_Cords[it->idxHelper].x >= probe)
					ItemsAfterEndCol.push_back(it);
			}
			mergeCol(cur, probe - 1, ItemsInCol, ItemsAfterEndCol, t_Cords, t_binWidth);
			cur++;
			probe = cur + 1;
			continue;
		}
...
*/
bool StripPacking::BLEU::mergeItems4yCheck(std::vector<item*>& t_allItems, item* t_i, std::map<int, std::list<item*>>& t_leftItems,
	...
	if (t_left)
	{
		...
		while (!t_Items.empty())
		{
			...
			if (*(std::max_element(allHeights.begin(), allHeights.end())) <= t_i->height)
			{
				...
				{
					this->merging(t_i, t_allItems, t_Cords, startColumn, maxWidth, t_Items, true);
					this->releaseTmpItems(transferredItems);
					return true;
				}
				else this->releaseTmpItems(transferredItems);
			}
			startColumn = this->getFirstColumn(t_Items, t_Cords);
			maxWidth = this->getMaxWidth(startColumn,t_Items, t_Cords, true);
			while (!this->checkSeparable(t_Items, t_Cords, startColumn, maxWidth, true))  // means it can be spearated
			{
			...
			while (ptr != t_Items.end())
			{
				...
				if (!(startColumn + maxWidth < p_is + (*ptr)->width && p_is + (*ptr)->width <= p_js))
				{
	...
	else
	{
		...
		while (!t_Items.empty())
		{
			...
			if (*(std::max_element(allHeights.begin(), allHeights.end())) <= t_i->height)
			{
				this->transferItemsAndCords4YEnumeration(t_Items, t_Cords, transferredItems, transferredCords);
				if (this->yCheckEnumerationTree(transferredItems, transferredCords, t_i->height,
					LastColumn - maxWidth - (t_Cords[t_i->idxHelper].x + t_i->width -1)) == solutionStatus::feasible)
				{
					this->merging(t_i, t_allItems, t_Cords, LastColumn, maxWidth, t_Items, false);
					this->releaseTmpItems(transferredItems);
					return true;
				}
				else this->releaseTmpItems(transferredItems);
			...
			while (!this->checkSeparable(t_Items, t_Cords, LastColumn, maxWidth,false))  // means it can be spearated
			{
...
const bool StripPacking::BLEU::checkSeparable(const std::list<item*>& t_Items, const std::vector<coordinate>& t_Cords,
	const int t_startColumn, const int t_maxWidth, const bool t_left) const
{
	if (t_left)
	{
		for (const auto it : t_Items)
		{
			...
			if (p_is<t_startColumn + t_maxWidth && p_is + it->width > t_startColumn + t_maxWidth)
				return false;
		}
		return true;
	}
	else
	{
		...
		{
			auto& p_is = t_Cords[it->idxHelper].x;
			if (p_is + it->width > t_startColumn - t_maxWidth && p_is < t_startColumn - t_maxWidth)
				return false;
		}
		return true;
	}
...
const int StripPacking::BLEU::getFirstColumn(const std::list<item*>& t_lefts, const std::vector<coordinate>& t_Cords) const
{
...
const int StripPacking::BLEU::getFirstColumn(const int t_prevStartColumn, const std::list<item*>& t_lefts, const std::vector<coordinate>& t_Cords) const
{
	if (t_lefts.empty()) return -1;
	std::vector<int> xCords;
	for (const auto& it : t_lefts)
...
const int StripPacking::BLEU::getLastColumn(const std::list<item*>& t_rights, const std::vector<coordinate>& t_Cords) const
{
...
const int StripPacking::BLEU::getLastColumn(const int t_prevLastColumn, const std::list<item*>& t_rights,
...
*/
const int StripPacking::BLEU::getMaxWidth(const int t_column, const std::list<item*>& t_Items, const std::vector<coordinate>& t_Cords, bool t_left) const
{
	if (t_left)
	{
...
void StripPacking::BLEU::transferItemsAndCords4YEnumeration(const std::list<item*>& t_OrigItems, const std::vector<coordinate>& t_OrigCords,
...
void StripPacking::BLEU::merging(item* t_i, std::vector<item*>& t_allItems, std::vector<coordinate>& t_Cords,
	const int t_startColumn, const int t_maxWidth, std::list<item*>& t_Items, const bool t_left) const
{
	...
	else
	{
		...
		t_i->width += (t_startColumn - t_maxWidth - (t_Cords[t_i->idxHelper].x + t_i->width - 1));
		auto ptr = t_allItems.begin();
		// 2) update allItems
		while (ptr != t_allItems.end())
		{
...
void StripPacking::BLEU::mergeItems(item* t_i, std::list<item*>& t_Items, std::map<int, std::list<item*>>& t_allItems,
...
*/
const std::vector<std::map<int, std::list<StripPacking::item*>>> StripPacking::BLEU::getLeftsAndRights(const std::vector<item*>& t_allItems,
...
void StripPacking::BLEU::releaseTmpItems(std::vector<const item*>& t_Items) const
{
	for (const auto & it : t_Items)
	{
